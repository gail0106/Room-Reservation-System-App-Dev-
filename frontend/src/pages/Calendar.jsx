...
function renderEventContent(eventInfo) {
  ...
  return (
    <div style={{
      ...
      background: status.bg, borderLeft: `3px solid ${status.border}`,
...
function EventModal({ event, onClose }) {
  ...
  const status = statusColors[event.extendedProps?.status] ?? statusColors.pending;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Colored top bar — retains status color */}
        ...
        <div style={{ padding: "1.25rem 1.5rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.25rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <p style={{ fontSize: 15, fontWeight: 600, margin: "0 0 2px", color: "#3A2000" }}>{event.title}</p>
                <span style={{
            ...
            <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#B0A080", fontSize: 18, padding: 0, lineHeight: 1, flexShrink: 0 }}>
              <i className="ti ti-x" />
            </button>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {event.extendedProps?.purpose && (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "0.75rem 1rem", background: "#FDF5DF", borderRadius: 8, border: "0.5px solid #EDE4D4" }}>
                <i className="ti ti-notes" style={{ fontSize: 15, color: "#C9991A", flexShrink: 0, marginTop: 1 }} />
                <div>
                  <p style={{ fontSize: 10, color: "#B0A080", margin: "0 0 1px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Purpose</p>
                  <p style={{ fontSize: 13, fontWeight: 500, margin: 0, color: "#3A2000" }}>{event.extendedProps.purpose}</p>
                </div>
              </div>
            )}
          </div>
  ...
  const fetchReservations = async () => {
    try {
      ...
      const colored = response.data.map((ev) => {
        const s = statusColors[ev.status] ?? statusColors.pending;
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: "1rem", flexWrap: "wrap" }}>
          {Object.entries(statusColors).map(([status, { bg }]) => (
            <div key={status} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 11, color: "#7A6030", textTransform: "capitalize" }}>{status}</span>
            </div>
          ))}
        </div>
            ...
            <>
              <style>{`
                ...
                .fc-button {
                  ...
                  border: 0.5px solid #D9C9A0 !important;
